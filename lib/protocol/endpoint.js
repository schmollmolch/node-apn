...
module.exports = function(dependencies) {
  ...
  function Endpoint(options) {
    ...
    if (options.proxy){
      ...
      if(this.options.proxy.username){
        // Basic auth as supported by node.js http.request
        this.options.auth = this.options.proxy.username+':'+this.options.proxy.password
      }
    }
  ...
  Endpoint.prototype._connect = function connect() {
    ...
    if(this.options.auth){
      http_options.auth = this.options.auth;
    }
    // Connecting through an HTTP proxy
    const req = http.request(http_options);
    req.end();
